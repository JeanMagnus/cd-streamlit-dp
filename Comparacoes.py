...
if aba == "Tratamento x Histórico Familiar":
    ...
    fig = px.bar(cross_tab, x='family_history', y='value', color='treatment',
                 title="Tratamento x Histórico Familiar", labels={'value': 'Quantidade'})
    st.plotly_chart(fig)
    ...
    fig = px.bar(remote_treat, x='remote_work', y='value', color='treatment',
                 title="Trabalho Remoto x Tratamento", labels={'value': 'Quantidade'})
    st.plotly_chart(fig)
    ...
    fig = px.bar(faixa_treat, x='faixa_etaria', y='value', color='treatment',
                 title="Faixa Etária x Tratamento", labels={'value': 'Quantidade'})
    st.plotly_chart(fig)

elif aba == "Gênero x Tratamento":
    genero_treat = pd.crosstab(df['Gender_clean'], df['treatment']).reset_index().melt(id_vars='Gender_clean')
    fig = px.bar(genero_treat, x='Gender_clean', y='value', color='treatment',
                 title="Gênero x Tratamento", labels={'value': 'Quantidade'})
    st.plotly_chart(fig)
