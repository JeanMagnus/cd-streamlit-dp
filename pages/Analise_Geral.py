...
# Gráfico de histórico familiar (sem alteração na legenda, pois já estava abaixo)
with col1:
...
# Gráfico de tratamento (COM A CORREÇÃO)
with col2:
    ...
    # --- AJUSTE DA LEGENDA E MARGEM ---
    fig_tratamento.update_layout(
        legend=dict(orientation="h", yanchor="bottom", y=1.02, xanchor="right", x=1),
        margin=dict(t=100) # Adiciona 100 pixels de margem no topo (t=top)
    )
    st.plotly_chart(fig_tratamento, use_container_width=True)

col3, col4 = st.columns(2)

# Gráfico de benefícios (sem alteração na legenda, pois já estava abaixo)
with col3:
...
# Gráfico de boxplot (idade x tratamento) (COM A CORREÇÃO)
with col4:
    ...
    fig_box.update_layout(
        legend=dict(orientation="h", yanchor="bottom", y=1.02, xanchor="right", x=1),
        margin=dict(t=80) # Adiciona 80 pixels de margem no topo (t=top)
    )
